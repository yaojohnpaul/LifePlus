	...
	public ArrayList<Task> getDailyQuests() {
		...
		if (c.moveToFirst()) {
			do {
				...
				String difficulty = c.getString(c.getColumnIndex(Task.COLUMN_DIFFICULTY));
				String duration = c.getString(c.getColumnIndex(Task.COLUMN_DURATION));
				String time = c.getString(c.getColumnIndex(Task.COLUMN_TIME));
				
				int generated = c.getInt(c.getColumnIndex(Task.COLUMN_GENERATED));
				boolean isGenerated;
				if(generated == 1){
					isGenerated = true;
				}
				...
				daily.add(new Task(id, name, desc, difficulty, duration, time, 1, isGenerated, isFinished));
			} while (c.moveToNext());
		}
	...
	public ArrayList<Task> getTodoList() {
		...
		if (c.moveToFirst()) {
			do {
				...
				try {
					date = new SimpleDateFormat("yyyyMMdd", Locale.ENGLISH).parse(c.getString(c.getColumnIndex(Task.COLUMN_DATE)));
				...
				todo.add(new Task(id, name, desc, difficulty, duration, date, time, 2, false, isFinished));
			} while (c.moveToNext());
		}
